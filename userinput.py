...
class Uinput:
    ...
    __slots__ = ['screen', 'rect', 'font', 'max_length', 'text_colour', 'background_colour', 'active_colour',
                 'frame', 'frame_colour', 'input_surface', 'format_input', 'length', 'text_x', 'text_y']
    ...
    def __init__(self, screen, rect, font, max_length, text_colour=BLACK, background_colour=LIGHT,
        ...
        self.format_input = {"n": self._no_format, "u": self._upper_case,
                             "l": self._lower_case, "p": self._proper_case}
        self.length = None
        text_height = self.font.render("Rlyg", True, self.text_colour).get_height()
        self.text_x, self.text_y = self.rect.x + self.frame * 2, self.rect.y + (self.rect.height - text_height) // 2
    ...
    def show_input(self, input_string, col=None):
    ...
    @staticmethod
    def _no_format(string):
        """ Show input as entered """
        return string

    @staticmethod
    def _upper_case(string):
